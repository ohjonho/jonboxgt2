...
const MinigameController = {
    ...
    showQuantumLeap() {
        ...
        // Phase 1: Pick a leap position (1-10)
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            btn.textContent = i;
            btn.onclick = () => {
                gameState.quantumPositions.set(gameState.currentPlayer.id, i);
                DOM.getAll('#quantumPositions .position-btn').forEach(b => b.classList.remove('selected'));
                btn.classList.add('selected');
                // Phase 2: Place a trap
                setTimeout(() => this.showQuantumTrapPhase(i), 500);
            };
        ...
        TimerManager.start('quantumTimer', CONFIG.TIMER_DURATION.quantum, () => {
            if (!gameState.quantumPositions.has(gameState.currentPlayer.id)) {
                ...
                this.showQuantumTrapPhase(rand);
            }
        });
    },
...
document.addEventListener('DOMContentLoaded', function() {
    setInterval(async () => {
        if (gameState.isHost && gameState.room && ConnectionManager.status === 'online') {
