        ...
        for (const api of STATIC_DATA.TRIVIA_APIS) {
            try {
                ...
                if (response.ok) {
                    ...
                    const processed = window[api.process](data);
    ...
    async fetchQuestionsFromAnyApi() {
        ...
        for (const api of workingApis) {
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    const processedQuestions = window[api.process](data);
    ...
    try {
        ...
        if (gameState.supabaseSubscription && gameState.room) {
            try {
                ...
                Utils.log(`Error syncing after adding AI player: ${error.message}`, 'warn');
            }
        }
...
const MinigameController = {
    ...
    showHuntingSeason() {
        ...
        TimerManager.start('huntingTimer', CONFIG.TIMER_DURATION.hunting, () => {
            if (!gameState.minigameSelections.has(gameState.currentPlayer.id)) {
                ...
                this.selectHuntingSquare(randomSquare);
            }
        });
    },
    ...
    showQuantumLeap() {
        ...
        setTimeout(() => this.showMinigameResults(['Quantum Leap completed!'], new Map(), new Map()), 2000);
    },

    showResourceRace() {
        // Implementation would go here
        setTimeout(() => this.showMinigameResults(['Resource Race completed!'], new Map(), new Map()), 2000);
    },

    showDiceDuel() {
        // Implementation would go here
        setTimeout(() => this.showMinigameResults(['Dice Duel completed!'], new Map(), new Map()), 2000);
    },

    showCodebreaker() {
        // Implementation would go here
        setTimeout(() => this.showMinigameResults(['Codebreaker completed!'], new Map(), new Map()), 2000);
    }
};
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // Add room code click-to-copy functionality
        const roomCodeEl = DOM.get('roomCodeDisplay');
        if (roomCodeEl) {
            roomCodeEl.onclick = () => {
                if (gameState.room) {
                    shareRoom();
